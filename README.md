...
This is part of a telemedicine project together with two other repositories whose purpose is the supervision from the patient’s home of a chronic disease, in our case Epilepsy. This part of the project is the desktop Java application used by a patients to record their data (ECG and EMG) using a BITalino which is then stored in a remote server.
...
This application can run in every operating system (Windows, OSX, Linux...) but the connection with BITalino only works in Windows, because the libraries. If this program will run in a Doctor computer this will not be a problem, but if the program need the BITalino connection to register data, as in Patients computers, only can be used with Windows.
...
<div align="center">
<img src="readme/resources/tree-tableview.png" width="600" border-radius=5% padding=100% />  
</div>  


7.[**BITalino Recording**]
To start to record your physiological data you have to connect your BITalino first. So if you don't, back to the last step to take it.
...
In the following screenshot we can observe data about an ECG patient test, but we can do the same patient test with an EMG.

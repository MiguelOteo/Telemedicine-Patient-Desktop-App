/**
 ...
 - launch your IDE;	
 - make sure that you have the Eclipse Integration Plugin installed (if you haven't, go to "File > Settings > Plugins > Install JetBrains Pluginâ€¦" and install the plugin);
 - to import the Java API project go to "File > Newâ€¦ > From Existing Sourcesâ€¦", then select your project folder in the dialog window, choose the .project file and press "OK";
 - make sure that the â€œSelect Eclipse projects directory:â€� field is the actual path of your project folder and then press "Next";
 - if unselected, select the project â€œAPI_BITalinoâ€� in order to import it and press "Next";
 ...
 - now that the project has been successfully imported, go to "File > Project Structureâ€¦", under "Project Settings", select "Modules" and click on API_BITALINO; 
 - at this point, in the "Dependencies" tab, make sure that the Module SDK is the same that you choose to your project;
 - then remove the â€œReferenced Librariesâ€� by selecting and pressing the minus icon on the left of the window;
 - press "Apply" and then "OK" in the bottom of the dialog;
 - under the API_BITALINO folder, select the "src" folder.
 - click on the test.java file with the right button of your mouse and select the â€œRun â€˜test.main()â€™â€�.
...
public class BITalino {
	...
	/// Array with the list of analog inputs to be acquired from the device
	/// (auxiliary variable)
	...
	/// Instance of the Bluetooth socket connection established with the BITalino
	/// device
	...
	/// Instance of the data stream through which data can be sent to the BITalino
	/// device
	...
	public void open(String macAdd) throws Throwable {
		/**
		 * Connects to a %BITalino device. \param[in] macAdd The device Bluetooth MAC
		 ...
		 * \exception IllegalArgumentException \exception ConnectionNotFoundException
	...
	public void open(String macAdd, int samplingRate) throws BITalinoException {
		/**
		 * Connects to a %BITalino device. \param[in] macAdd The device Bluetooth MAC
		 * address ("xx:xx:xx:xx:xx:xx") \param[in] samplingRate Sampling rate in Hz.
		 * Accepted values are 1, 10, 100 or 1000 Hz. Default value is 1000 Hz.
		 * 
		 * \exception BITalinoErrorTypes (BITalinoErrorTypes.MACADDRESS_NOT_VALID)
		 * \exception BITalinoErrorTypes (BITalinoErrorTypes.SAMPLING_RATE_NOT_DEFINED)
		 * \exception IllegalArgumentException \exception ConnectionNotFoundException
	...
	public void start(int[] anChannels) throws Throwable {
		/**
		 * Starts a signal acquisition from the device. \param[in] anChannels Set of
		 * channels to acquire. Accepted channels are 0...5 for inputs A1...A6. If this
		 * set is empty, no analog channels will be acquired. \remarks This method
		 * cannot be called during an acquisition. \exception BITalinoException
		 * (BITalinoErrorTypes.ANALOG_CHANNELS_NOT_VALID) \exception BITalinoException
	...
	public void stop() throws BITalinoException {
		/**
		 * Stops a signal acquisition. \remarks This method must be called only during
		 * an acquisition. \exception BITalinoException
		 * (BITalinoErrorTypes.BT_DEVICE_NOT_CONNECTED)
		 */
		try {
			Write(0);
	...
	public void close() throws BITalinoException {
		/**
		 * Disconnects from a %BITalino device. If an aquisition is running, it is
		 * stopped. \exception BITalinoException
		 * (BITalinoErrorTypes.BT_DEVICE_NOT_CONNECTED)
		 */
		try {
			hSocket.close();
	...
	public void Write(int data) throws BITalinoException {
		/**
		 * Send a command to BITalino \param[in] data Byte corresponding to the command
		 * to be sent to the %BITalino device \exception BITalinoException
	...
	public void battery(int value) throws BITalinoException {
		/**
		 * Sets the battery voltage threshold for the low-battery LED. \param[in] value
		 * Battery voltage threshold. Default value is 0. Value | Voltage Threshold
		 * ----- | ----------------- 0 | 3.4 V ... | ... 63 | 3.8 V \remarks This method
		 * cannot be called during an acquisition. \exception BITalinoException
		 * (BITalinoErrorTypes.THRESHOLD_NOT_VALID) \exception BITalinoException
		 * (BITalinoErrorTypes.LOST_COMMUNICATION)
		 */
		int Mode;
		if (value >= 0 && value <= 63) {
	...
	public void trigger(int[] digitalArray) throws BITalinoException {
		 ...
		 * integers to assign to digital outputs, starting at first output (O1). On each
		 * vector element, 0 sets the output to low level and 1 sets the output to high
		 * level. This vector must contain exactly 4 elements. \remarks This method must
		 * be called only during an acquisition on original %BITalino. On %BITalino 2
		 * there is no restriction. \exception BITalinoException
		 * (BITalinoErrorTypes.DIGITAL_CHANNELS_NOT_VALID) \exception BITalinoException
		 * (BITalinoErrorTypes.LOST_COMMUNICATION)
		 */
	...
	public String version() throws BITalinoException, IOException {
		/**
		 * Returns the device firmware version string. \remarks This method cannot be
		 * called during an acquisition. \exception BITalinoException
	...
	private Frame[] decode(byte[] buffer) throws IOException, BITalinoException {
		/**
		 * Unpack a raw byte stream into a frames vector. \param[in] buffer Vector with
		 * the bytes read from the device. \return Vector of frames decoded frames.
		...
		try {
			...
			if (CRC == ((x3 << 3) | (x2 << 2) | (x1 << 1) | x0)) {
				...
				case 4:
					...
					frames[i].analog[4] = (short) ((((buffer[j - 6] & 0x0F) << 2) | ((buffer[j - 7] & 0xc0) >> 6))
							& 0x3f);
				case 3:

					frames[i].analog[3] = (short) ((((buffer[j - 5] & 0x3F) << 4) | ((buffer[j - 6] & 0xf0) >> 4))
							& 0x3ff);
				case 2:

					frames[i].analog[2] = (short) ((((buffer[j - 4] & 0xff) << 2) | (((buffer[j - 5] & 0xc0) >> 6)))
							& 0x3ff);
				case 1:
				...
				case 0:
					...
					frames[i].analog[0] = (short) ((((buffer[j - 1] & 0xF) << 6) | ((buffer[j - 2] & 0XFC) >> 2))
							& 0x3ff);
				}
	...
	public Frame[] read(int nSamples) throws BITalinoException {
		/**
		 * Reads acquisition frames from the device. This method returns when all
		 * requested frames are received from the device, or when a receive timeout
		 * occurs. \param[in] nSamples Number of frames that should be read from the
		 * device. \return Vector of frames obtained from the device. \remarks If a
		 * problem occurred, the size of the frames vector is lower than the frames
		 * vector size. This method must be called only during an acquisition.
